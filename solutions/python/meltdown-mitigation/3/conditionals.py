...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    # return 'green' if efficiency>=80 else 'orange' if efficiency<80 and efficiency>=60 else 'red' if efficiency>=30 and efficiency<60 else 'black'
    if efficiency >= 80:
        return 'green'
    elif efficiency >= 60:
        return 'orange'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if 0.9 * threshold > power:
        return 'LOW'
    elif 0.9 * threshold <= power <= threshold * 1.1:
        return 'NORMAL'
    return 'DANGER'
